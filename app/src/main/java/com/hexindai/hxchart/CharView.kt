...
//        canvas.translate(-900f, 0F)
        val pointPosition = getPointPosition()
        //话最前面的区域
        ...
        path.lineTo(pointPosition[pointPosition.size - 1].x.toFloat(), pointPosition[pointPosition.size - 1].y.toFloat())
        path.lineTo(pointPosition[pointPosition.size - 1].x.toFloat(), height.toFloat())
    ...
    private fun getPointPosition(): ArrayList<Point> {
        //这个就是最高点
        ...
        val canUseWidth = width - padding * 2

        Log.e("CharView", width.toString())
        Log.e("CharView", height.toString())
        val itemWidth = canUseWidth / maxPoint

        total.forEachIndexed { index, valueAndText ->
            arrayList.add(Point(padding + itemWidth / 2 + itemWidth * index, (((1 - valueAndText.value / maxValuePoint.value) * canUseHeight) + topPading).toInt()))
